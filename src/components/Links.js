...
const Links = ({ person, setperson, location, refe }) => {
    ...
    const deleteChild = (index) => {
        ...
        if (window.confirm("Deleting this item?")) {
                    ...
                    person.children.map((i, index) => {
                        return <div key={index} style={{ display: "flex", flexDirection: "column", justifyContent: "space-between", margin: "20px" }}>
                            <div key={`/add?path=${location}/${index}`}
                                className="alert" style={{ position: "relative", opacity: "1", marginTop: "0", display: "flex", flexDirection: "column", justifyContent: "space-between" }}
                                onClick={(e) => {

                                }
                                }>
                                {i.imageURL &&
                                    <img src={i.imageURL} style={{ position: "absolute", top: 75, right: 35 }} className="round-img logo" alt="l" />
                                }
